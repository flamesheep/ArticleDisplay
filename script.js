document.addEventListener('DOMContentLoaded', () => {
  ...
  function createItemHTML(item) {
    ...
    div.innerHTML = `
        <div class="info">
            <span>${item.date}</span> | <strong>${item.title}</strong>
            ${item.r18 ? '<span style="color:red;"> [R18]</span>' : ''}
        </div>
        <button class="link-btn">閱讀文章</button>
    `;

    const btn = div.querySelector('.link-btn');
    btn.addEventListener('click', () => {
        if (item.r18) {
            const pwd = prompt("此文章涉及成人內容，請輸入密碼：");
            if (pwd === PASSWORD) {
